...
simulations = 100;	### Number of simulations of the process
...
for (j in 1:simulations) {   # Loop through each simulation
  ...
  for (i in 2:length(t) ) {  # Loop through the time values
    ...
    stochasticPart=p*(1-p/K)*sigma*sqrt(dt)*rnorm(1);  ### Assume the population growth term is stochastic
    
    # Now update each of the determinsitc dynamics
    dpdt = r*p*(1-p/K);   # dp/dt
