...
if (estimateMode=='superfast') {
  ...
  NUM_AT_ONCE = 1000 #NEEDED
  NCHAINS = 1 # NEEDED
  BURN_ITER = 7000 # NEEDED Number of runs we do after convergences, for burn-in
  ESTIMATE_ITER = 8000 # Number of runs that we actually record data
  MAX_ITER = 15000  # NEEDED
...
for (i in 1:NCHAINS) {
	...
	write(paste("Start Chain Time:", date()),outFileName,append=TRUE)
		...
		while ( abs(acceptVal-A_STAR)>THRESH & iterCurr < MAX_ITER ) {
			...
			write(c(paste("Iteration: ", iterCurr, round(acceptVal*100,digits=3), "% accepted at ", date())), outFileName,append=TRUE)
	...
	write(c(paste("Parameter", "CurrentValue", "MaxLLvalue","Knob")), outFileName,append=TRUE)
		...
		write(c(paste("Acceptance (%): ", round(acceptVal*100,digits=3))), outFileName,append=TRUE)
