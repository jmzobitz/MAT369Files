...
parameters = c(delta = 0.01, beta = 0.001)   # parameters: a named vector
initialCondition = c(S = 1, R = 3)   # initial condition.  Be sure to name your variables

### 2) Time values
deltaT = 0.1    # timestep length
timeSteps = 5000   # must be a number greater than 1 
...
dynamics <- function(t, state, parameters){
  with(as.list(c(state, parameters)), {
    ...
    ### Your differential equation will go in the following lines
     dS = -delta * S 
     dR = delta * S - beta * R
    return(list(c(dS, dR)))    ### You might need to modify dV or dP depending on your variables
